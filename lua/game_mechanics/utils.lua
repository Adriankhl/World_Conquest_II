...
function wc2_utils.load_wc2_data()
	if wc2_utils.world_conquest_data = nil then
		local data_dict = {}
		local ignore_list = {}
		local active_resources = wc2_utils.split_to_array(wesnoth.game_config.active_resources)
		for i,res_id in ipairs(active_resources) do
			local ressource = wesnoth.get_ressourc(res_id)
