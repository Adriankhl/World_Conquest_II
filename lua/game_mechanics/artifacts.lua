...
function artifact.add_artifact_data(a)
	artifacts.list:insert(a)
end

function artifacts.read_wml_data(cfg)
	for i, artifact in wc2_convert.wml_to_lon(wml.literal(cfg), "wct_artifact_list").artifact or {} do
		artifact.add_artifact_data(artifact)
	end
end

function artifacts.get_artifact(id)
	return artifacts.list[index]
end
